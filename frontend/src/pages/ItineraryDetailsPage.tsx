  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-dark-bg pb-20">
            ...
            <div>
              <div className="flex flex-wrap gap-3 mb-4">
                ...
                <span className="px-3 py-1 bg-primary-500/20 backdrop-blur-md rounded-full text-sm font-medium text-primary-300 border border-primary-500/30 flex items-center">
                  <Brain className="w-4 h-4 mr-1.5" /> {itinerary.match_score}% Profile Match
                </span>
              </div>
              <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-2">{itinerary.title}</h1>
              <p className="text-slate-300 text-lg">AI-Optimized Route & Energy Plan</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 relative z-20">
        ...
        {/* Stats Row */}
        <div className="bg-white dark:bg-dark-surface rounded-2xl shadow-xl border border-slate-100 dark:border-dark-border p-6 grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
          <div>
            <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mb-1">Total Estimated Cost</p>
          ...
          <div>
            <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mb-1">Attractions</p>
            <p className="text-2xl font-bold text-slate-900 dark:text-white flex items-center">
          ...
          <div>
            <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mb-1">Avg Daily Energy</p>
            <p className="text-2xl font-bold text-slate-900 dark:text-white flex items-center">
              <Zap className="w-5 h-5 mr-2 text-amber-500" />
              {itinerary.avg_daily_energy.toFixed(1)} <span className="text-sm font-normal text-slate-400 ml-1">/10</span>
            </p>
          </div>
          <div>
            <p className="text-sm text-slate-500 dark:text-slate-400 font-medium mb-1">Pace Assessment</p>
            <p className="text-lg font-bold text-slate-900 dark:text-white">
              {itinerary.avg_daily_energy > 8 ? 'High Intensity' : itinerary.avg_daily_energy < 5 ? 'Relaxed' : 'Moderate'}
            </p>
          </div>
        </div>
        ...
        {itinerary.warnings && itinerary.warnings.length > 0 && (
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 p-4 rounded-xl mb-8 flex items-start">
            ...
            <div>
              <h4 className="font-semibold text-amber-800 dark:text-amber-400">AI Notes</h4>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-300 dark:before:via-slate-700 before:to-transparent">
              {currentDayData?.slots.map((slot: any, index: number) => (
                ...
                >
                  {/* Timeline Node */}
                  <div className="flex items-center justify-center w-10 h-10 rounded-full border-4 border-white dark:border-dark-bg bg-primary-100 dark:bg-slate-800 text-primary-600 dark:text-primary-400 shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2">
                    {slot.slot_type === 'attraction' ? <MapPin className="w-4 h-4" /> : 
                     slot.slot_type === 'lunch' || slot.slot_type === 'dinner' ? <span className="text-sm">🍽️</span> : 
                     <Clock className="w-4 h-4" />}
                  </div>

                  {/* Card */}
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-2xl bg-white dark:bg-dark-surface shadow-md border border-slate-100 dark:border-dark-border">
                    <div className="flex justify-between items-start mb-2">
                      <span className="text-sm font-bold text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/20 px-2 py-1 rounded">
                        {slot.time_label}
                      </span>
                    ...
                    {slot.slot_type === 'attraction' && slot.recommendation_reason && (
                      <div className="mt-3 p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl border border-indigo-100 dark:border-indigo-800/30 flex items-start">
                        <Brain className="w-4 h-4 text-indigo-500 mr-2 shrink-0 mt-0.5" />
                        <p className="text-xs text-indigo-900 dark:text-indigo-300">
                          <span className="font-semibold block mb-0.5">Why you'll love it:</span>
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-dark-surface p-6 rounded-2xl shadow-xl border border-slate-100 dark:border-dark-border sticky top-24">
              <div className="flex items-center mb-4">
                ...
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">AI Reasoning</h3>
              </div>
              
              <p className="text-sm text-slate-600 dark:text-slate-400 mb-6 pb-6 border-b border-slate-100 dark:border-slate-800">
                This itinerary was uniquely generated for your profile. Here's a breakdown of some key decisions the model made.
              </p>
